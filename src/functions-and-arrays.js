...
function findLongestWord(words1) {
        ...
        for ( let i = 0; i < words1.length; i++){
          if (words1[i].length > words1[i+1].length){

             return longestWord = words1[i]

          } else if (words1[i + 1].length > words1[i].length)
            
            return longestWord = words1[i + 1];
        }


}

let longestWord1 = findLongestWord(words);
...
function averageWordLength(avarr) {
  let total2 = 0;

  for (let i =0; i < avarr.length; i++){
      total2 += avarr[i].length;
  }
  let averageWordLengthOfArray = total2 / avarr.length;
  return averageWordLengthOfArray;

}
   let resultAverageWord = averageWordLength(wordsArr)
  console.log(resultAverageWord);
 

// Bonus - Iteration #4.1
const mixedArr5 = [6, 12, 'miami', 1, true, 'barca', '200', 'lisboa', 8, 10];
function avg(mixedArray) {
    let total4 = 0;

    for(let i = 0; i < mixedArray.length; i++){
       if (typeof(mixedArray[i]) === 'strings'){
        total4 += mixedArray[i].length
       }

       else if (typeof(mixedArray[i]) === 'boolean'){
        total4 += mixedArray[i] ? 1 : 0
       }
    ...
    let   mixAverage = total4 / mixedArray.length;
...
let FinalMIxAverage = avg(mixedArr5)
console.log(FinalMIxAverage);
...
function uniquifyArray(duplicateArray) {
   ...
   for (let i =0; i < duplicateArray.length; i++){
       if (newArray.indexOf(duplicateArray[i]) === -1){
...
let uniqueArray = uniquifyArray(wordsUnique);
